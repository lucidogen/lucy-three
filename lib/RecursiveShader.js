...
RecursiveShader.prototype.setup = function(options, sub) {
  ...
  if ( ! self.makeTarget2 )
  { self.makeTarget2 = function ( w, h )
    { if ( self.target2 ) self.target2.dispose ()
      ...
      if ( self.target2bis ) self.target2bis.dispose ()
    ...
    self.renderer.onResize ( self.makeTarget2 )
  }

  self.makeTarget2 ( window.innerWidth, window.innerHeight )
